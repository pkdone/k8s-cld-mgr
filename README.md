...
1. First the Docker image containing the Cloud Manger Automation Agent and associated entrypoint "wrapper" Bash script (start.sh) needs to be generated into an image, using the command-line terminal/shell with the following commands:
...
1. In the YAML resource file that defines the Kubernetes StatefulSet (**'resources/mongodb-agent-service.yaml'**) replace the following text elements with the corresponding values for your environment and save the changes:
...
1. Log-on to your [Cloud Manager account](https://www.mongodb.com/cloud/cloud-manager) and check that the three Kubernetes provisioned Automation Agents are appearing and showing as 3 Servers, listed in the Deployments section (similar to the screenshot below).
